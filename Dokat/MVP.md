...
Cho phép người dùng gửi ảnh thú cưng vừa chụp đến một người bạn cụ thể trong
danh sách bạn bè.

### Users / Actors

- **Sender:** Người dùng gửi ảnh.
- **Recipient:** Người bạn nhận ảnh.
...
2. Hệ thống SHALL cho phép Sender chọn đúng một người nhận trong một lần gửi.
3. Hệ thống SHALL upload ảnh lên S3 và lưu metadata post vào PostgreSQL trước
   khi thông báo cho Recipient.
4. Hệ thống SHALL gửi push notification đến Recipient ngay sau khi ảnh được
   lưu thành công (xem F09).
5. Hệ thống SHALL cập nhật feed của Recipient để ảnh mới xuất hiện ở đầu feed
   (xem F06).
6. Hệ thống SHALL lưu timestamp gửi vào metadata của post.
7. Hệ thống SHALL hiển thị trạng thái "Đã gửi" cho Sender sau khi upload thành
   công.
8. Hệ thống SHALL xử lý trường hợp mất kết nối: retry upload tối đa 3 lần
   trước khi báo lỗi cho người dùng.

### Non-goals

- Không hỗ trợ gửi đến nhiều người cùng lúc (multi-select recipient) trong MVP.
- Không hỗ trợ gửi nhóm (group send) trong MVP.
...
- Sau khi upload thành công, client gọi FastAPI để tạo post record trong
  PostgreSQL.
...
**AC-F05-1:** Gửi ảnh thành công
```
Given Sender đã chụp ảnh hợp lệ và chọn 1 người nhận
When Sender nhấn "Gửi"
Then ảnh xuất hiện trên feed của Recipient trong vòng 5 giây
  và Recipient nhận được push notification
  và Sender thấy trạng thái "Đã gửi"
```

**AC-F05-2:** Mất kết nối khi gửi
```
...
Then hệ thống tự động retry upload
  và ảnh được gửi thành công mà không cần Sender thao tác lại
```

**AC-F05-3:** Upload thất bại sau 3 lần retry
```
