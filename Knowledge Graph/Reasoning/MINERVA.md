...
- Đây là **directed labeled multigraph**: cạnh có chiều, có nhãn relation, và có thể có nhiều cạnh khác nhãn nối cùng hai node.  
...
Trực giác: relation query `LocatedIn` không chỉ là “nhãn cần dự đoán”; nó là **mục đích điều hướng**. Cùng đứng ở một node, nếu câu hỏi là `WorksFor` thì agent nên ưu tiên quan hệ dẫn tới tổ chức/công ty; nếu là `BornIn` thì ưu tiên đường qua location.  
...
Do đó inference chỉ xét các cạnh lân cận dọc các path được policy chọn. Nó vừa có khả năng tạo reasoning chain nhiều bước, vừa trả về path như một dạng **provenance / lời giải có thể diễn giải**.  
...
Điểm này giải thích khác biệt với DeepPath. DeepPath tìm path khi đã biết cả source lẫn target, nên phù hợp hơn với việc xác minh fact. MINERVA khó hơn: target bị che giấu và agent phải khám phá graph để suy ra target.  

## MDP: biến suy luận thành điều hướng

MINERVA mô hình hóa KB-QA thành một **finite-horizon deterministic partially observable Markov decision process** (POMDP/MDP quan sát một phần) trên graph.  
...
Tại training, môi trường biết $e_2^q$ để tính reward. Nhưng agent **không được nhìn thấy** $e_2^q$, bởi test time không có đáp án. Đây là nguyên nhân bài toán là *partially observable*.  
...
Nó không biết $e_2^q$. Nếu agent thấy target, bài toán sẽ biến thành route finding “đến node đã biết”, thay vì QA thực sự.  
...
Đây không phải chỉ để tăng dữ liệu. Nó có chức năng điều hướng quan trọng: agent đi nhầm có thể quay lại node trước thay vì bị kẹt. Thí nghiệm minh họa agent có thể đi sai ở bước đầu, dùng inverse edge quay lại, rồi đi theo relation đúng để đến đáp án.  
...
Paper dùng **LSTM** để nhớ path history, embedding cho entity/relation, rồi dùng MLP để chấm điểm các action hợp lệ.  
...
Paper dùng embedding relation và entity kích thước 200 trong các thí nghiệm chính.  
...
Dấu $[\, ; \,]$ là phép **concatenation**, nên vector action có kích thước $2d$. Trực giác: chỉ relation là chưa đủ, vì hai cạnh cùng relation có thể dẫn đến hai entity rất khác nhau; chỉ destination cũng chưa đủ, vì relation đã đi mang ý nghĩa reasoning riêng.  
...
- $[\mathbf{a}_{t-1}; \mathbf{o}_t]$: input tại step $t$, ghép “vừa đi qua quan hệ gì” với “hiện đứng ở entity nào”  
...
Mỗi hàng là embedding $[\mathbf{r}_l;\mathbf{e}_d]$ của một outgoing edge.  
...
Thay vì train một classifier hoặc rule-set cho từng query relation, MINERVA train **một policy chung**. Nhờ đó, knowledge học từ relation phổ biến có thể hỗ trợ relation ít dữ liệu hơn, đồng thời không phải maintain hàng nghìn model khi graph có nhiều relation type.  
...
Nói đơn giản: tối ưu $\theta$ sao cho nếu lấy một fact training, cho agent khởi hành từ head/source và chọn đường đi theo policy, xác suất kết thúc đúng tail/answer cao nhất có thể.  
...
Nếu rollout đạt đáp án, $R=1$, gradient tăng probability của toàn bộ action sequence đã dẫn tới thành công. Nếu thất bại, reward thấp, policy giảm xu hướng lặp lại trajectory đó.  
...
- Baseline không đổi expected gradient, nhưng giảm variance, giúp train ổn định hơn.  

Paper dùng 20 rollouts cho mỗi training example để xấp xỉ expectation trên các trajectory mà policy có thể sinh ra.  
...
5. Lặp lại đến khi policy học được path patterns tổng quát.  
...
- Entity không được beam reach nhận rank vô cực.  

Beam search thay sampling ngẫu nhiên bằng một chiến lược tìm kiếm có kiểm soát, phù hợp khi cần top-$k$ answers và đánh giá theo ranking metrics.  
...
MINERVA mạnh khi dữ liệu cần reasoning path và relation pattern lặp lại; không phải lúc nào cũng thắng embedding model, nhất là graph nhỏ hoặc benchmark thiên về link-prediction thống kê.  

| Dataset / thiết lập | Kết quả đáng chú ý |
|---|---|
| COUNTRIES S3 | MINERVA đạt AUC-PR 95.10, cao hơn các baseline được báo cáo; S3 là task đòi hỏi reasoning khó nhất trong bộ này.    |
| WN18RR | MINERVA đạt HITS@1 0.413, cao hơn ComplEx 0.382, ConvE 0.403 và DistMult 0.410; HITS@10 là 0.513.    |
| NELL-995 | MINERVA đạt HITS@1 0.663 và HITS@10 0.831; cạnh tranh với embedding baselines và vượt path-baseline rõ rệt.    |
| FB15K-237 | MINERVA HITS@10 0.456, thấp hơn ConvE 0.600; paper giải thích tập này có nhiều one-to-many relation và benchmark chỉ chọn một target triple, không hoàn toàn phù hợp với endpoint-based QA.    |
| WikiMovies | Với câu hỏi tự nhiên dạng template, MINERVA đạt accuracy 96.7, cao hơn NeuralLP 94.6 trong bảng paper.    |
| Gridworld | Agent giảm ít hơn NeuralLP khi path length tăng, cho thấy khả năng xử lý reasoning chain dài.    |
